...
### 3.4 Key Results: The "All-in-One" Backbone
DINOv2 achieves strong performance on "Frozen Features"—meaning the backbone is *never* updated. Its capabilities can be categorized by task type, which dictates the optimal embedding usage:

*   **Global Tasks (Classification, Retrieval)**:
    *   **Performance**: Reaches **86.5% top-1** on ImageNet with a linear probe, matching state-of-the-art weakly supervised models.
    *   **Usage**: Use the **`[CLS]` token** (or average pool), which aggregates global semantics.
*   **Dense Prediction (Segmentation, Depth)**:
    *   **Performance**: Unlike CLIP (good at text but weak at localization), DINOv2 produces clear object boundaries and understands 3D geometry.
    *   **Usage**: Use **Patch Tokens**. Thanks to the iBOT objective, these tokens preserve precise spatial information.
*   **Universal Representation**: The same frozen backbone serves all these downstream tasks simultaneously.
